...
public class SasAgent extends Player {
     ...
     */
    public SasAgent(long seed, int id) {
        super(seed, id);
        reset(seed, id);
    }
    ...
    @Override
    public Player copy() {
        SasAgent player = new SasAgent(seed,playerID);
    ...
    public Types.ACTIONS act(GameState gs) {
        ...
        Types.ACTIONS actionToReturn = Types.ACTIONS.ACTION_STOP;
        ...
        if(!unsafeDirections.isEmpty()){
            ...
            if(!directions.isEmpty()) {
                ...
                actionToReturn = Types.ACTIONS.ACTION_STOP;
            }
        }
        ...
        if(isAdjacentEnemy(items, dist, enemies) && maybeBomb(ammo, blastStrength, items, dist, myPosition)){
            actionToReturn =  Types.ACTIONS.ACTION_BOMB;
        }
        ...
        for (GameObject en: enemies){
            ...
            while (dist_it.hasNext()){
                ...
                if (entry.getKey().equals(en.getPosition()) && entry.getValue() == 3){
                    ...
                    actionToReturn =  directionToAction(getDirection(myPosition, next_node));
        ...
        if (distance <= 2){
            ...
            if (myPosition.equals(previousNode)){
                actionToReturn =  directionToAction(getDirection(myPosition, previousNode));
            }
                ...
                for (Vector2d coords:entry.getValue()){
                    ...
                    if (dist.get(coords) <= 2){
                        ...
                        if (direction != null){
                            ...
                            if (dirArray.size() > 0){
                                actionToReturn =  directionToAction(dirArray.get(0));
                            }
                        }
        ...
        if (validDirections.size() > 0){
            ...
            actionToReturn =  directionToAction(validDirections.get(actionIdx));
        }


        CustomHeuristic rootStateHeuristic = new CustomHeuristic(gs);



        GameState gsCopy = gs.copy();
        double valStateBefore = rootStateHeuristic.evaluateState(gsCopy);
        rollRnd(gsCopy, actionToReturn);
        double valStateAfter = rootStateHeuristic.evaluateState(gsCopy);

        if (valStateAfter>valStateBefore) return actionToReturn;
        else return Types.ACTIONS.ACTION_STOP;
